    ...
    if (driver_setup == DRIVER_SETUP_OK) {
    ...
    #endif
        {
            ...
            if ( (quantum_msg[1] & 0x20 )) {    // Release
                ...
                bChangeUpDn= 1;
		   touch_state_val = 0;
		   printk(KERN_DEBUG "[TSP]### Finger[%d] Up (%d,%d) - fingerid=%d status=0x%02x\n", id, fingerInfo[id].x, fingerInfo[id].y , id, quantum_msg[1]);
            }
                ...
                printk(KERN_DEBUG "[TSP]### Finger[%d] Down (%d,%d) - fingerid=%d status=0x%02x\n", id, fingerInfo[id].x, fingerInfo[id].y , id, quantum_msg[1]);
