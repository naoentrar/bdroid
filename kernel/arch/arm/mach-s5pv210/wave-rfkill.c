...
static int __init wave_rfkill_probe(struct platform_device *pdev)
{
	int irq;
	int ret;
...
static struct platform_driver wave_device_rfkill = {
	.probe = wave_rfkill_probe,
	.driver = {
		.name = "bt_rfkill",
		.owner = THIS_MODULE,
	},
...
static int __init wave_rfkill_init(void)
{
	int rc = 0;
	rc = platform_driver_register(&wave_device_rfkill);

	return rc;
}

module_init(wave_rfkill_init);
MODULE_DESCRIPTION("wave rfkill");
MODULE_LICENSE("GPL");
