...
void s5pv210_setup_sdhci0_cfg_gpio(struct platform_device *dev, int width)
{
	...
	printk(KERN_DEBUG "configuring shdci0 with width=%d\n",width);
	switch (width) {
...
/*TODO check if line below does apply to WAVE*/
...
void s5pv210_setup_sdhci1_cfg_gpio(struct platform_device *dev, int width)
{
	...
	printk(KERN_DEBUG "configuring shdci1 with width=%d\n",width);
	switch (width) {
...
void s5pv210_setup_sdhci2_cfg_gpio(struct platform_device *dev, int width)
{
	...
	printk(KERN_DEBUG "configuring shdci2 with width=%d\n",width);
	switch (width) {
...
void s5pv210_setup_sdhci3_cfg_gpio(struct platform_device *dev, int width)
{
	...
	printk(KERN_DEBUG "configuring shdci3 with width=%d\n",width);
	switch (width) {
...
//#endif
	...
	printk(KERN_DEBUG "Universal : Card status %d <hardcoded 1>\n", card_status ? 0 : 1);
	return 1/*card_status ? 0 : 1*/; //hardcoded 1 for now, S8530 detect pin may differ.
